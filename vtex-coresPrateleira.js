...
(function($){
	...
	jQuery.fn.coresPrateleira=function(opts)
	{
		...
		log=function(b,a){"object"===typeof console&&("undefined"!==typeof a&&"alerta"===a.toLowerCase()?console.warn("["+extTitle+"]\n"+b):"undefined"!==typeof a&&"info"===a.toLowerCase()?console.info("["+extTitle+"]\n"+b):console.error("["+extTitle+"]\n"+b))};
		debug=function(b,a){"object"===typeof console&&debugOn&&("undefined"!==typeof a&&"alerta"===a.toLowerCase()?console.warn("[Debug]["+extTitle+"]\n"+b):"undefined"!==typeof a&&"info"===a.toLowerCase()?console.info("[Debug]["+extTitle+"]\n"+b):console.error("[Debug]["+extTitle+"]\n"+b))};
		...
		fn=
		{
					...
					if(fn.options.useProductField)
					{
						...
						if(debugOn)
						{
							if(productField.find("li").text().trim()==="")
								debug("O campo produto não esta retornando nenhum valor.\n Produto: "+($this.find(".vtex-cpProductLink[title]:first").attr("title")||"[Título não encontrado]"),"info");
						}
			...
			getProductInfo:function(callback,prodId,prodUrl,stop,skuInfo,$firstData,relatedWrapper)
			{
				...
				jQuery.ajax({
					...
					{
						var skuId,prodSpecifications=[],specNumbers=[],specNumbersInArray,jsonSpecifications,inArray=-1;
						...
						// Agurpando as especificações dos produtos
						if(fn.options.skuGroupSelector !== null)
							$data.find(fn.options.skuGroupSelector).each(function(){
								var spec=$(this).val()||"";
								if(spec!=="")
									prodSpecifications.push(spec);
							});
						...
						for(i=0;i<lg;i++)
							for(var k in skuSpecifications.skus[i]){
								...
								// Verifica se foi passado parametro para verificar o grupo de SKU
								if(fn.options.skuGroupSelector !== null){
									jsonSpecifications=k.split(",");
									
									// Obtem o número da especificação correspondente ao grupo passado por parametro. Em resumo verifica o ID da especificação de COR (exemplo)
									for(var spec in  jsonSpecifications){
										inArray=$.inArray(jsonSpecifications[spec],prodSpecifications);
										if(inArray>-1)
											break;
									}
									
									// Caso a especificação seja encontrada ela é adicionada ao Array de especificações já processadas e o script continua sua execução
									specNumbersInArray=$.inArray(prodSpecifications[inArray],specNumbers);
									if(inArray>-1 && specNumbersInArray < 0)
										specNumbers.push(prodSpecifications[inArray]);
									// Caso falhe este SKU é ignorado
									else{
										if(specNumbersInArray>-1)
											debug("O SKU "+skuId+" foi ignorado pois seguindo as especificações obtidas na página de produto através do seletor “"+fn.options.skuGroupSelector+"” ele tem as mesmas imagens que outro SKU já existente na lista.","info");
										else
											debug("O SKU "+skuId+" foi ignorado pois nenhuma das especificações obtidas na página de produto através do seletor “"+fn.options.skuGroupSelector+"” coincidem com as dele que são “"+k+"”","info");
										continue;
									}
								}
...
$(function(){
    $(".prateleira").coresPrateleira({checkDuplicateUri:false});
    $(document).ajaxStop(function(){ $(".prateleira").coresPrateleira({checkDuplicateUri:false}); });
});